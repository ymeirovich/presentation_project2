...
def _uuid_type():
    """Return a UUID-compatible column type for the active dialect."""
    bind = op.get_bind()
...
def _array_of_strings():
    """Return an array-like column type for string lists per dialect."""
    bind = op.get_bind()
    if bind.dialect.name == "sqlite":
        return sa.JSON()
...
def _timestamp_default_now():
    """Cross-dialect NOW() default."""
    bind = op.get_bind()
    if bind.dialect.name == "sqlite":
...
def upgrade() -> None:
    ...
    op.create_table('certification_profiles',
        ...
        sa.Column('prerequisites', _array_of_strings(), nullable=True),
        sa.Column('recommended_experience', sa.Text(), nullable=True),
        sa.Column('is_active', sa.Boolean(), nullable=False, default=True),
        sa.Column('created_at', sa.DateTime(timezone=True), server_default=_timestamp_default_now(), nullable=False),
        sa.Column('updated_at', sa.DateTime(timezone=True), server_default=_timestamp_default_now(), nullable=False),
    ...
    op.create_table('knowledge_base_documents',
        sa.Column('id', _uuid_type(), nullable=False),
        sa.Column('certification_profile_id', _uuid_type(), nullable=False),
    ...
    op.create_table('presentation_generations',
        ...
        sa.Column('rag_sources_used', _array_of_strings(), nullable=True),
    ...
    op.create_table('assessment_results',
        ...
        sa.Column('rag_sources_referenced', _array_of_strings(), nullable=True),
        sa.Column('created_at', sa.DateTime(timezone=True), server_default=_timestamp_default_now(), nullable=False),
        sa.Column('updated_at', sa.DateTime(timezone=True), server_default=_timestamp_default_now(), nullable=False),
        sa.PrimaryKeyConstraint('id'),
        sa.ForeignKeyConstraint(['certification_profile_id'], ['certification_profiles.id'], ondelete='CASCADE')
    ...
    op.create_table('learning_content',
        sa.Column('id', _uuid_type(), nullable=False),
        sa.Column('assessment_result_id', _uuid_type(), nullable=False),
        sa.Column('content_type', sa.String(length=50), nullable=False),
        sa.Column('title', sa.String(length=200), nullable=False),
        sa.Column('content_outline', sa.JSON(), nullable=False),
        sa.Column('slide_count', sa.Integer(), nullable=False),
        sa.Column('estimated_duration_minutes', sa.Integer(), nullable=False),
        sa.Column('target_gaps', _array_of_strings(), nullable=True),
        sa.Column('rag_context_sources', _array_of_strings(), nullable=True),
        sa.Column('generation_status', sa.String(length=50), nullable=False),
        sa.Column('google_slides_url', sa.Text(), nullable=True),
        sa.Column('created_at', sa.DateTime(timezone=True), server_default=_timestamp_default_now(), nullable=False),
        sa.Column('updated_at', sa.DateTime(timezone=True), server_default=_timestamp_default_now(), nullable=False),
    ...
    op.create_table('identified_gaps',
        ...
        sa.Column('evidence_sources', _array_of_strings(), nullable=True),
