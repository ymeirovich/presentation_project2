...
from presgen-training.src.hardware_profiler import HardwareProfiler, ResourceMonitor
from presgen-training.src.avatar_generator import AvatarGenerator
from presgen-training.src.video_utils import VideoUtils
...
class TrainingVideoOrchestrator:
    ...
    def process_training_video(
        ...
        try:
            ...
            script_content, audio_path = self._phase2_script_processing(script_path)
            if not audio_path:
    ...
    def _phase2_script_processing(self, script_path: Path) -> tuple[str, Optional[Path]]:
        """Phase 2.1: Process script and generate TTS audio"""
        jlog(self.logger, logging.INFO,
        ...
        # Generate TTS audio using avatar generator
        audio_path = self.avatar_generator.generate_tts_audio(
            script_content, 
            self.output_dir / f"tts_audio_{self.job_id}.wav"
        )
